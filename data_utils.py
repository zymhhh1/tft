...
FEAT_ORDER = [
        ...
        (InputTypes.TARGET, DataTypes.CATEGORICAL),
...
class TFTBinaryDataset(Dataset):
    def __init__(self, path, config):
        ...
        print(self.feature_type_col_map)
        #raise ValueError("~")
        ...
        #self.group_num = len(config.quantiles)
        #print("-------------------------------------------")
        #print([arr.shape for arr in self.grouped[0]])
        #for i in range(len(self.grouped)):
        #    self.grouped[i][-2] = np.array([np.eye(self.group_num)[label] for label in self.grouped[i][-2]])
...
class TFTDataset(Dataset):
    def __init__(self, path, config):
        ...
        self.grouped = list(self.data.groupby(DEFAULT_ID_COL))
