...
MAKE_CONVERT_COMPATIBLE = False # os.environ.get("TFT_SCRIPTING", None) is not None
...
class StaticCovariateEncoder(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
        ...
        with open('attention_weight/sparse_weights.npy', 'wb') as f:
            np.save(f, sparse_weights.detach().cpu())
        # Context vectors:
...
class TFTBack(nn.Module):
    ...
    def forward(self, historical_inputs, cs, ch, cc, ce, future_inputs):
        historical_features, h_vsn = self.history_vsn(historical_inputs, cs)
        with open('attention_weight/h_vsn.npy', 'wb') as f:
            np.save(f, h_vsn.detach().cpu())
        history, state = self.history_encoder(historical_features, (ch, cc))
        future_features, f_vsn = self.future_vsn(future_inputs, cs)
        with open('attention_weight/f_vsn.npy', 'wb') as f:
            np.save(f, f_vsn.detach().cpu())
        ...
        x, attn = self.attention(enriched)
        with open('attention_weight/attn.npy', 'wb') as f:
            np.save(f, attn.detach().cpu())
