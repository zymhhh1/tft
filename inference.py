...
def _unscale_per_id(config, values, ids):
...
def _unscale(config, values):
    ...
    for col in flat_values.columns:
        if not 'id' in col:
            ...
            # _t_col = scaler.inverse_transform(_col)[:,-1]
            flat_values[col] = _col

    flat_values = flat_values[[col for col in flat_values if not 'id' in col]]
    return flat_values.values

def predict(args, config, model, data_loader, extend_targets=False):
    model.eval()
    predictions = []
    targets = []
    ids = []
    ...
    if config.scale_per_id:
        ...
        unscaled_predictions = np.stack(
                [_unscale_per_id(config, predictions[:,:,i], ids) for i in range(len(config.quantiles))], 
                axis=-1)
        unscaled_targets = np.expand_dims(_unscale_per_id(config, targets[:,:,0], ids), axis=-1)
    else:
        ids = None
        unscaled_predictions = np.stack(
                [_unscale(config, predictions[:,:,i]) for i in range(len(config.quantiles))], 
                axis=-1)
        unscaled_targets = np.expand_dims(_unscale(config, targets[:,:,0]), axis=-1)
